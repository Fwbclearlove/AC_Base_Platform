# src/03_generate_remaining_summaries.py
import os
import json
import time
...
class RemainingSummaryGenerator:
    """剩余文档概要生成器"""
...
def main():
    ...
    if os.path.exists(SUMMARIES_DATA_PATH):
        try:
            with open(SUMMARIES_DATA_PATH, 'r', encoding='utf-8') as f:
                ...
                existing_summaries = set(summaries.keys())
                print(f"已有概要: {len(existing_summaries)} 个")
        except:
            print("概要文件读取失败，将创建新的")
    ...
    for file_id, info in metadata.items():
        # 跳过已有概要的文件
        if file_id in existing_summaries:
            continue

        # 检查是否有基础提取的文件
        if (info.get("status") == "text_extracted" and
                info.get("extraction_method") == "basic_reliable" and
                "text_path" in info):

            text_path = info["text_path"]
            if os.path.exists(text_path):
                remaining_files[file_id] = {
                    **info,
                    "text_path": text_path
                }
            else:
                print(f"警告：文件路径不存在 {text_path}")
    ...
    print(f"找到 {len(remaining_files)} 个待处理的文档")

    generator = RemainingSummaryGenerator()
    ...
    # 开始生成概要
    for file_id, info in tqdm(remaining_files.items(), desc="生成剩余概要"):
        ...
        try:
            ...
            print(f"处理文件: {file_id} (长度: {len(document_text)} 字符)")
            ...
            if summary:
                ...
                summary["source_type"] = "basic_extracted"
    ...
    if generator.total_count > 0:
        print(
            f"   成功率: {generator.success_count}/{generator.total_count} ({generator.success_count / generator.total_count * 100:.1f}%)")

    print(f"\n概要已追加到: {SUMMARIES_DATA_PATH}")
    print(f"单独文件保存在: {SUMMARIES_DIR}")
    print(f"现在总共有 {len(all_summaries)} 个文档概要")
